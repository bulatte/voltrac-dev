  ...
  return (
    <footer className={cn("border-t border-t-white-25 relative")}>
      ...
      <Container className="relative">
        <div className="px-[var(--cxp)] py-cyp flex flex-col gap-20 lg:gap-40">
          <ContactForm />
          <FooterLogo className="w-full h-auto" />
        </div>
      </Container>
      <Container className="border-t border-t-white-25 relative">
        <div className="px-[var(--cxp)] py-7.5 flex flex-col md:flex-row items-center md:items-end justify-between gap-5 md:gap-10">
          <nav className="grid md:flex flex-wrap gap-x-5 w-full md:w-auto grid-cols-1 text-center md:text-left">
            {menu.map((item) => (
              <Link
                className={cn(
